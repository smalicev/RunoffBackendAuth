...
class ChicagoStorm {
    ...
    generateTab() {
        ...
        this.intensity = this.generatePrePeak()
            .reverse()
            .concat(this.generatePostPeak());
        this.sortTime();
    ...
    generatePrePeak() {
        ...
        for (
            let i = this.timestep;
            i <= this.roundToTimestep(this.peaktime);
            i += this.timestep
        ) {
            intensity.push(
                (
                    (this.a * ((i / this.peaktime) * (1 - this.c) + this.b)) /
                    (i / this.peakRatio + this.b) ** (1 + this.c)
                ).toFixed(2)
            );
    ...
    generatePostPeak() {
        ...
        for (
            let i = 0;
            i <=
            this.roundToTimestep(this.duration) -
                this.roundToTimestep(this.peaktime);
            i += this.timestep
        ) {
            intensity.push(
                (
                    (this.a *
                        ((i / (1 - this.peakRatio)) * (1 - this.c) + this.b)) /
                    (i / (1 - this.peakRatio) + this.b) ** (1 + this.c)
                ).toFixed(2)
            );
    ...
    generateLastPoint() {
        ...
        if (this.time[this.time.length - 1] < this.duration) {
            ...
            this.intensity.push(
                (
                    (this.a *
                        ((timeAfterPeak / (1 - this.peakRatio)) * (1 - this.c) +
                            this.b)) /
                    (timeAfterPeak / (1 - this.peakRatio) + this.b) **
                        (1 + this.c)
                ).toFixed(2)
            );
        }
